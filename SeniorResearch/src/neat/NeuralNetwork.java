...
public class NeuralNetwork {
	...
	private Map<Integer, Neuron> neurons; // All neurons in organism, mapped by ID
	...
	public NeuralNetwork(Organism organism) {
		...
		for (Integer nodeID : organism.getNodeGenes().keySet()) {
			NodeGene node = organism.getNodeGenes().get(nodeID);
		...
		for (Integer connID : organism.getConnectionGenes().keySet()) {
			ConnectionGene conn = organism.getConnectionGenes().get(connID);
			if (!conn.isExpressed()) {
				continue;
			}
	...
	public float[] calculate(float[] input_parameter) {
		...
		if (input_parameter.length != input.size()) {
			throw new IllegalArgumentException("Number of inputs must match number of input neurons in organism");
		}
